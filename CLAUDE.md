...
- `packages/shared` — source of truth for **all** WS payload shapes
  (zod schemas + inferred types) and for the raw-code → display-name table
  in `src/maps.ts` (`bigmap` → `Customs`, etc.). Never duplicate either in
  `apps/client` or `apps/server`.
