...
describe('tarkovTime', () => {
  it('at epoch is Moscow midnight + 3h (left) / +15h (right)', () => {
    expect(tarkovTime(0)).toEqual({ left: '03:00', right: '15:00', seconds: '00' });
  });

  it('advances 7× real time', () => {
    // 1 real hour → 7 in-game hours.
    expect(tarkovTime(1 * HOUR)).toEqual({ left: '10:00', right: '22:00', seconds: '00' });
  });

  it('wraps past midnight', () => {
    // 3 real hours → 21 in-game hours; 03:00 + 21h = 00:00.
    expect(tarkovTime(3 * HOUR)).toEqual({ left: '00:00', right: '12:00', seconds: '00' });
  });
